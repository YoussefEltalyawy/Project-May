...
async function fetchCompoundPugViewJson(
  cid: string,
  toc?: string,
...
function textsFromSections(
  ...
  return Array.from(new Set(all.filter((t) => t.length >= minLen))).slice(
    0,
    limit,
  );
}
...
function extractPictogramCodesFromSections(sections: unknown[]): string[] {
  ...
  return normalizeGhsPictogramCodes(Array.from(codes)).sort((a, b) =>
    a.localeCompare(b),
  );
}
...
function parseGHSFromRoots(views: unknown[]): GHSData {
  ...
  const hazSec = findSectionMany(
    views,
    "ghs hazard statements",
    "hazard statements",
    "hazard statement codes",
  );
  const precSec = findSectionMany(
    views,
  ...
  const hazPool = hazSec.length
    ? collectStringsFromNodes(hazSec)
    : collectStringsFromNodes(views);
  const precPool = precSec.length
    ? collectStringsFromNodes(precSec)
    : collectStringsFromNodes(views);
...
function finalizeGhs(views: unknown[], ghs: GHSData): GHSData {
  ...
  const hazardStatements = dedupeHazardStatements(ghs.hazardStatements).slice(
    0,
    120,
  );
  const corpus: string[] = [];
  ...
  return {
    ...
    pictograms,
    hazardStatements,
    precautionaryStatements: flatLines.length
      ? flatLines
...
export async function autocompleteCompoundNames(
  query: string,
  limit = 10,
): Promise<string[]> {
  const q = query.trim();
  ...
  try {
    const res = await fetchJsonWithRetry(
      `${AUTOCOMPLETE}/${encodeURIComponent(q)}/json?limit=${limit + 4}`,
    );
    if (!res.ok) return [];
    const data = await res.json();
...
export async function getChemicalIdentity(
  cid: string,
): Promise<ChemicalIdentity> {
  ...
  try {
    ...
    if (propRes.ok) {
      ...
      result.molecularWeight = p.MolecularWeight
  ...
  try {
    const synRes = await fetchJsonWithRetry(
      `${PUG}/compound/cid/${cid}/synonyms/JSON`,
    );
    if (synRes.ok) {
...
function getPhysicalPropertiesFromViews(views: unknown[]): PhysicalProperties {
  ...
  for (const [key, headings] of headingMap) {
    const sections = findSectionMany(
      views,
...
export async function fetchFullSDSByCid(
  cid: string,
  fallbackName?: string,
): Promise<SDSData | null> {
  ...
  const storage = getSectionTextsFromViews(
    views,
    6,
    60,
    "Storage",
