...
if (typeof process !== "undefined" && process.env) {
  (process.env as any).NODE_SET_DEFAULT_AUTO_SELECT_FAMILY_ATTEMPT_TIMEOUT =
    "1000";
}
...
if (!connectionString) {
  console.warn(
    "[DB] No connection string found (DATABASE_URL or POSTGRES_URL). Caching will be disabled.",
  );
}
 ...
 */
export async function retrySql<T>(
  op: () => Promise<T>,
  retries = 2,
): Promise<T | null> {
  if (!sql) return null;
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (i < retries) {
        console.warn(
          `[DB] Operation failed, retrying (${i + 1}/${retries})...`,
          (e as Error).message,
        );
