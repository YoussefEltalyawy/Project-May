 ...
 */
function convertPhysicalToMetric(
...
function truncatePrompt(prompt: string, maxTokens: number): string {
  ...
  if (truncationPoint > maxChars * 0.6) {
    return (
      prompt.substring(0, truncationPoint) +
  ...
  for (const modelName of models) {
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction:
            "You are a professional safety documentation assistant. You strictly output JSON formatting an SDS summary.",
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert Safety Data Sheet (SDS) compiler. Respond ONLY with valid JSON.",
            },
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are a professional safety data officer. Output ONLY valid JSON.",
                },
                {
                  role: "user",
                  content:
...
export async function POST(request: Request) {
  try {
    ...
    if (cid) {
      try {
        ...
        if (existing && Array.isArray(existing) && existing.length > 0) {
          console.log(
            `[Cache Hit] Using cached summary for: ${chemicalName} (CID: ${cid})`,
          );
          // Ensure we return a plain object, not a database row proxy
          // Strip preparedBy — it must be entered fresh each time
          const { preparedBy: _omit, ...cleanData } = existing[0]
            .data as Record<string, unknown>;
          return NextResponse.json(cleanData);
        }
      } catch (dbError) {
        console.error(
          "Database lookup error (skipping cache):",
    ...
    const sectionsRaw = {
      ...
      fireFighting: (data.fireFighting?.text || []).map((t) =>
        expandGHSCodes(t),
      ),
      accidentalRelease: (data.accidentalRelease?.text || []).map((t) =>
        expandGHSCodes(t),
      ),
    ...
    const chunks = [
      {
        ...
        input: {
          ...
          hazards: sectionsRaw.hazards,
          composition: sectionsRaw.composition,
        },
        schema: {
          physical: {
            type: "object",
      ...
      },
      {
        id: "Safety Measures",
        input: {
          firstAid: sectionsRaw.firstAid,
          fireFighting: sectionsRaw.fireFighting,
          accidentalRelease: sectionsRaw.accidentalRelease,
        },
        schema: {
      ...
      },
      {
        id: "Handling & Health",
        input: {
          handling: sectionsRaw.handling,
          storage: sectionsRaw.storage,
          exposure: sectionsRaw.exposure,
        },
        schema: {
      ...
      },
      {
        ...
        input: {
          stability: sectionsRaw.stability,
          toxicology: sectionsRaw.toxicology,
        },
        schema: {
      ...
      },
      {
        ...
        input: {
          ecological: sectionsRaw.ecological,
          disposal: sectionsRaw.disposal,
          transport: sectionsRaw.transport,
          regulatory: sectionsRaw.regulatory,
          otherInfo: sectionsRaw.otherInfo,
        },
        schema: {
      ...
      },
      {
        ...
        input: {
          hazards: sectionsRaw.hazards.slice(0, 5),
          firstAid: sectionsRaw.firstAid.slice(0, 3),
        },
        schema: {
...
6. Keep it under 25 words. DO NOT say "check english instructions".`,
    ...
    const results = await Promise.all(
      chunks.map(async (chunk) => {
        ...
        const prompt =
          (chunk as Chunk).customPrompt ||
          `You are an expert Safety Data Sheet (SDS) compiler working on a chemical named "${chemicalName}".
...
`;
        ...
        const aiPromise = (async () => {
          ...
          if (!chunkResult && process.env.GEMINI_API_KEY1)
            chunkResult = await tryGemini(
              prompt,
              responseSchema,
              process.env.GEMINI_API_KEY1,
            );
          if (!chunkResult && process.env.GEMINI_API_KEY2)
            chunkResult = await tryGemini(
              prompt,
              responseSchema,
              process.env.GEMINI_API_KEY2,
            );
          if (!chunkResult && process.env.GEMINI_API_KEY3)
            chunkResult = await tryGemini(
              prompt,
              responseSchema,
          ...
          if (!chunkResult && process.env.GROQ_API_KEY)
            chunkResult = await tryGroq(prompt);
          if (!chunkResult && process.env.OPENROUTER_API_KEY)
            chunkResult = await tryOpenRouter(prompt);
          return chunkResult;
        })();
    ...
    const outputJson = results.reduce<Record<string, unknown>>(
      (acc, curr) => ({ ...acc, ...curr }),
      {},
    );

    // Extract typed values from outputJson
    const physicalOutput = outputJson.physical as
    ...
    const summarizedData: SDSData = {
      ...
      physical: {
        appearance:
          physicalOutput?.appearance ??
          data.physical?.appearance ??
          "No data available",
        boilingPoint:
          physicalOutput?.boilingPoint ??
          data.physical?.boilingPoint ??
          "No data available",
        meltingPoint:
          physicalOutput?.meltingPoint ??
          data.physical?.meltingPoint ??
          "No data available",
        flashPoint:
          physicalOutput?.flashPoint ??
          data.physical?.flashPoint ??
          "No data available",
        density:
          physicalOutput?.density ??
          data.physical?.density ??
          "No data available",
        vaporPressure:
          physicalOutput?.vaporPressure ??
          data.physical?.vaporPressure ??
          "No data available",
        solubility:
          physicalOutput?.solubility ??
          data.physical?.solubility ??
          "No data available",
        ph: physicalOutput?.ph ?? data.physical?.ph ?? "No data available",
        odor:
          physicalOutput?.odor ?? data.physical?.odor ?? "No data available",
        autoIgnition:
          physicalOutput?.autoIgnition ??
          data.physical?.autoIgnition ??
          "No data available",
      },
      hazards: {
        text: getStringArray("hazards")?.length
          ? getStringArray("hazards")!
          : ["Not classified as hazardous"],
      },
      composition: {
        text: getStringArray("composition")?.length
          ? getStringArray("composition")!
          : ["No additional ingredients identified."],
      },
      firstAid: {
        text: getStringArray("firstAid")?.length
          ? getStringArray("firstAid")!
          : ["No specific first aid measures required."],
      },
      fireFighting: {
        text: getStringArray("fireFighting")?.length
          ? getStringArray("fireFighting")!
          : ["Use extinguishing media appropriate for surrounding fire."],
      },
      accidentalRelease: {
        text: getStringArray("accidentalRelease")?.length
          ? getStringArray("accidentalRelease")!
          : ["No special requirements."],
      },
      handling: {
        text: getStringArray("handling")?.length
          ? getStringArray("handling")!
          : [
              "Handle in accordance with good industrial hygiene and safety practice.",
            ],
      },
      storage: {
        text: getStringArray("storage")?.length
          ? getStringArray("storage")!
          : ["Store in a cool, well-ventilated place."],
      },
      exposure: {
        text: getStringArray("exposure")?.length
          ? getStringArray("exposure")!
          : ["Contains no substances with occupational exposure limit values."],
      },
      stability: {
        text: getStringArray("stability")?.length
          ? getStringArray("stability")!
          : ["Stable under normal conditions."],
      },
      toxicology: {
        text: getStringArray("toxicology")?.length
          ? getStringArray("toxicology")!
          : ["No toxicological data available for this mixture."],
      },
      ecological: {
        text: getStringArray("ecological")?.length
          ? getStringArray("ecological")!
          : ["No environmental hazards identified."],
      },
      disposal: {
        text: getStringArray("disposal")?.length
          ? getStringArray("disposal")!
          : ["Dispose of in accordance with local regulations."],
      },
      transport: {
        text: getStringArray("transport")?.length
          ? getStringArray("transport")!
          : ["Not regulated for transport."],
      },
      regulatory: {
        text: getStringArray("regulatory")?.length
          ? getStringArray("regulatory")!
          : ["No additional regulatory information available."],
      },
      otherInfo: {
        text: getStringArray("otherInfo")?.length
          ? getStringArray("otherInfo")!
          : ["Created for safety documentation."],
      },
      arabicWarning:
        (outputJson.arabicWarning as string) ||
        "تحذير: يجب مراجعة تعليمات السلامة الإنجليزية لهذا المنتج بعناية والتعامل معه بحذر.",
    };

    // 2. Cache the result for future use (strip preparedBy before persisting)
    const { preparedBy: _omitSave, ...dataToSave } =
      summarizedData as SDSData & { preparedBy?: unknown };
    if (cid && sql) {
      try {
        await retrySql(() => {
          ...
          return sql`
            ...
            VALUES (${cid}, ${chemicalName}, ${dataToSave as unknown as Record<string, unknown>})
        ...
        console.log(
          `[Cache Save] Saved summary for: ${chemicalName} (CID: ${cid})`,
        );
      } catch (dbError) {
        console.error(
          "Failed to cache chemical data:",
          (dbError as Error).message,
        );
      }
    }
